...
The solution is to use the `Decimal` class from the `decimal` built-in module. The `Decimal` class provides fixed point math of 28 decimal points by default. It can go even higher it required. This words around the precision issues in IEEE 754 floating point numbers.
...
While `Decimal` works great for fixed point numbers, it still has limitations in its precision (e.g. 1/3 will be a approximation). For representing rational numbers with no limit to precision, consider using the `Fraction` class from the `fractions` built-in module.
